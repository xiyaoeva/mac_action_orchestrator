...
def get_chrome_tab_urls() -> Optional[List[str]]:
    script = r'''
...
'''
    rc, out, err = executor.run_osascript(script, timeout=10, rate_limit_seconds=0)
    if rc != 0:
        return None
    if not out:
        return []
...
def update_latest_tab_url(previous_urls: Optional[List[str]] = None) -> Optional[str]:
    global LAST_TAB_URLS, LAST_NEW_TAB_URL
    urls = get_chrome_tab_urls()
    if urls is None:
...
def screen_size():
    try:
        size, stdout, stderr, rc = executor.get_screen_size_debug()
        if not size:
            return JSONResponse(
                {
                    "ok": False,
        ...
        while idx < len(expanded_actions):
            ...
            if action.type == "click_at" and (action.x is None or action.y is None):
                ...
                if not LAST_SCREEN_PATH or not LAST_SCREEN_PATH.exists():
                    raise ValueError("No previous screenshot available for click_at target.")
                screen_size = executor.get_screen_size() or LAST_SCREEN_SIZE
                if not screen_size:
                ...
                if len(candidates) == 1:
                    ...
                    return JSONResponse({
                        ...
                        "target": action.target,
                        "count": len(candidates),
                        "prompt_used": req.context_prompt or action.target,
                        "results": results,
                        "latest_tab_url": update_latest_tab_url(previous_urls=prev_tab_urls),
                    })
                x = int(box["left"]) + int(box["width"]) // 2
                ...
                if (sw, sh) != (iw, ih):
                    x = int(round(x * (sw / iw)))
                    y = int(round(y * (sh / ih)))
                action = action.model_copy(update={"x": x, "y": y})
            ...
            if action.type == "plan_again":
                if req.pause_on_plan_again:
                    ...
                    local_path = new_screen_path()
                    executor.capture_screen_to_local(local_path, rate_limit_seconds=req.cooldown_seconds)
                    LAST_SCREEN_PATH = local_path
                    url = f"/static/screenshots/{local_path.name}"
                    return JSONResponse({
                        "ok": True,
                ...
                function_decls = build_action_function_declarations(allowed_targets=allowed_targets)
...
def call_gemini_planner(
    ...
    from google.genai import types
    function_decls = function_decls or build_action_function_declarations()
    tools = types.Tool(function_declarations=function_decls)
    config = types.GenerateContentConfig(tools=[tools])
    client = get_client()
    if image_path:
        parts = [
            types.Part.from_text(text=planner_system_prompt() + prompt),
            types.Part.from_bytes(data=image_path.read_bytes(), mime_type="image/png"),
        ]
        content = types.Content(role="user", parts=parts)
...
def plan_actions(req: PlanRequest):
    ...
    try:
        response = call_gemini_planner(req.prompt, model=model)
    except FileNotFoundError as e:
        return JSONResponse(
            {"ok": False, "error": f"API key file not found: {e}. Please create apikeys.txt with at least one key."},
            status_code=500,
        )
    except RuntimeError as e:
        msg = str(e)
        if "API key" in msg and ("为空" in msg or "empty" in msg or "不存在" in msg):
            return JSONResponse(
                {"ok": False, "error": f"API keys are missing or empty: {msg}. Please add keys to apikeys.txt."},
                status_code=500,
            )
        if "API key 列表为空" in msg:
            return JSONResponse(
                {"ok": False, "error": "API key list is empty or exhausted. Please add more keys to apikeys.txt."},
                status_code=500,
            )
